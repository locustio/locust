        ...
        if hasattr(task, "tasks"):
            ...
            if tags is not None:
                passing &= (
                    ...
                    and len(task.locust_tag_set & tags) > 0
                )
...
class TaskSet(object, metaclass=TaskSetMeta):
    ...
    def get_next_task(self):
        if not self.tasks:
            raise Exception(
                "No tasks defined. use the @task decorator or set the tasks property of the TaskSet"
            )
...
class DefaultTaskSet(TaskSet):
    ...
    def get_next_task(self):
        if not self.user.tasks:
            raise Exception(
                "No tasks defined. use the @task decorator or set the tasks property of the User"
            )
