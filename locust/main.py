...
def is_user_class(item):
    ...
    """
    return bool(
        inspect.isclass(item) and issubclass(item, User) and item.abstract == False
    )
...
def load_locustfile(path):
    ...
    user_classes = {
        name: value for name, value in vars(imported).items() if is_user_class(value)
    }

    # Find shape class, if any, return it
    shape_classes = [
        value for name, value in vars(imported).items() if is_shape_class(value)
    ]
    if shape_classes:
