...
from .stats import print_error_report, print_percentile_stats, print_stats, stats_printer, stats_writer, write_csv_files
...
def is_user_class(item):
    ...
    """
    return bool(inspect.isclass(item) and issubclass(item, User) and not item.abstract)
...
def main():
    ...
    try:
        ...
        logger.warning(
            "System open file limit setting is not high enough for load testing, and the OS didn't allow locust to increase it by itself. See https://docs.locust.io/en/stable/installation.html#increasing-maximum-number-of-open-files-limit for more info."
        )
    ...
    if options.csv_prefix:
        gevent.spawn(
            stats_writer, environment, options.csv_prefix, full_history=options.stats_history_enabled
