...
function renderTable(report) {
  ...
  window.alternate = false;
  $("#errors tbody").jqoteapp(
    errors_tpl,
    report.errors.sort(sortBy(failuresSortAttribute, failuresDesc))
  );
...
var responseTimeChart = new LocustLineChart(
  $(".charts-container"),
  "Response Times (ms)",
  ["Median Response Time", "95% percentile"],
  "ms"
);
var usersChart = new LocustLineChart(
  $(".charts-container"),
  "Number of Users",
  ["Users"],
  "users"
...
function updateStats() {
  $.get("./stats/requests", function (report) {
    ...
    try {
      ...
      stats_history["current_rps"].push({
        value: total.current_rps,
        users: report.user_count,
      });
      stats_history["current_fail_per_sec"].push({
        value: total.current_fail_per_sec,
        users: report.user_count,
      });
      stats_history["response_time_percentile_50"].push({
        value: report.current_response_time_percentile_50,
        users: report.user_count,
      });
      stats_history["response_time_percentile_95"].push({
        value: report.current_response_time_percentile_95,
        users: report.user_count,
      });
      update_stats_charts();
    } catch (i) {
