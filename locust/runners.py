...
(
    ...
    STATE_STOPPED,
    STATE_MISSING,
) = ["ready", "spawning", "running", "cleanup", "stopping", "stopped", "missing"]
...
class Runner(object):
    ...
    def weight_users(self, amount):
        ...
        if len(bucket) < amount:
            ...
            for user in [
                l
                for l, r in sorted(residuals.items(), key=lambda x: x[1], reverse=True)
            ][: amount - len(bucket)]:
                bucket.append(user)
        elif len(bucket) > amount:
            # We've got too many users due to rounding errors so we need to remove some
            for user in [l for l, r in sorted(residuals.items(), key=lambda x: x[1])][
                : len(bucket) - amount
            ]:
                bucket.remove(user)
    ...
    def stop_users(self, user_count, stop_rate=None):
        ...
        if stop_rate == None or stop_rate >= user_count:
            ...
            logger.info(
                "Stopping %i users at rate of %g users/s" % (user_count, stop_rate)
            )
    ...
    def start_stepload(self, user_count, spawn_rate, step_user_count, step_duration):
        ...
        self.stepload_greenlet = self.greenlet.spawn(
            self.stepload_worker, spawn_rate, step_user_count, step_duration
        )
    ...
    def stepload_worker(self, spawn_rate, step_users_growth, step_duration):
        ...
        while (
            ...
            logger.info(
                "Step loading: start spawn job of %d user" % (current_num_users)
            )
...
class MasterRunner(DistributedRunner):
    ...
    def __init__(self, environment, master_bind_host, master_bind_port):
        ...
        self.greenlet.spawn(self.heartbeat_worker).link_exception(
    ...
    def cpu_log_warning(self):
        ...
        if self.worker_cpu_warning_emitted:
            logger.warning(
                "CPU usage threshold was exceeded on workers during the test!"
            )
    ...
    def check_stopped(self):
        if not self.state == STATE_INIT and all(
            map(
                lambda x: x.state != STATE_RUNNING and x.state != STATE_SPAWNING,
                self.clients.all,
            )
        ):
            self.state = STATE_STOPPED
        ...
        while True:
            ...
            for client in self.clients.all:
                if client.heartbeat < 0 and client.state != STATE_MISSING:
                    logger.info(
                        "Worker %s failed to send heartbeat, setting state to missing."
                        % str(client.id)
                    )
            ...
            if msg.type == "client_ready":
                ...
                if msg.node_id in self.clients:
                    ...
                    if not c.cpu_warning_emitted and c.cpu_usage > 90:
                        self.worker_cpu_warning_emitted = (
                            True  # used to fail the test in the end
                        )
                        c.cpu_warning_emitted = (
                            True  # used to suppress logging for this node
                        )
                        logger.warning(
                            "Worker %s exceeded cpu threshold (will only log this once per worker)"
                            % (msg.node_id)
                        )
            elif msg.type == "stats":
                self.environment.events.worker_report.fire(
                    client_id=msg.node_id, data=msg.data
                )
                ...
                if msg.node_id in self.clients:
                    ...
                    if self.worker_count - len(self.clients.missing) <= 0:
                        ...
                        self.stop()
                        if (
                            self.environment.parsed_options
                            and self.environment.parsed_options.headless
                        ):
                            self.quit()
...
class WorkerRunner(DistributedRunner):
    ...
    def __init__(self, environment, master_host, master_port):
        ...
        def on_spawning_complete(user_count):
            self.client.send(
                Message("spawning_complete", {"count": user_count}, self.client_id)
            )
    ...
    def worker(self):
        while True:
            ...
            if msg.type == "spawn":
                ...
                self.spawning_greenlet = self.greenlet.spawn(
                    lambda: self.start(
                        user_count=job["num_users"], spawn_rate=job["spawn_rate"]
                    )
                )
                self.spawning_greenlet.link_exception(greenlet_exception_handler)
            elif msg.type == "stop":
