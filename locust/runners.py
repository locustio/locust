...
class LocustRunner(object):
    ...
    def get_locust_co(self, locust_co_index):
        return self._locust_cos.get(locust_co_index)
    ...
    def spawn_locusts(self, spawn_count=None, stop_timeout=None, wait=False):
        ...
        def hatch():
            ...
            while True:
                ...
                def start_locust(co_idx):
                    try:
                        locust_inst = locust()
                        setattr(locust_inst, '_locust_co_index', co_idx)
                        locust_inst.run()
                    except GreenletExit:
                        pass

                self._max_locust_co_index += 1
                new_locust_co_idx = self._max_locust_co_index

                new_locust_co = self.locusts.spawn(start_locust, new_locust_co_idx)
                self._locust_cos[new_locust_co_idx] = new_locust_co
    ...
    def client_listener(self):
        while True:
            ...
            if msg.type == "client_ready":
                ...
                logger.info(
                    "Client %r reported as ready. Currently %i clients ready to swarm." % (id, len(self.clients.ready)))
