...
def setup_distributed_stats_event_listeners(environment):
    def on_report_to_master(client_id, data):
        ...
        data["stats_total"] = environment.stats.total.get_stripped_report()
        data["errors"] =  environment.stats.serialize_errors()
        environment.stats.errors = {}
    
    def on_slave_report(client_id, data):
        for stats_data in data["stats"]:
            entry = StatsEntry.unserialize(stats_data)
            request_key = (entry.name, entry.method)
            if not request_key in environment.stats.entries:
        ...
        for error_key, error in six.iteritems(data["errors"]):
            if error_key not in environment.stats.errors:
                ...
                environment.stats.errors[error_key].occurrences += error["occurrences"]
        ...
        old_last_request_timestamp = environment.stats.total.last_request_timestamp
        # update the total StatsEntry
        environment.stats.total.extend(StatsEntry.unserialize(data["stats_total"]))
        if environment.stats.total.last_request_timestamp and environment.stats.total.last_request_timestamp > (old_last_request_timestamp or 0):
