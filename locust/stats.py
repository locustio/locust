...
CachedResponseTimes = namedtuple(
    "CachedResponseTimes", ["response_times", "num_requests"]
)
...
class RequestStats(object):
    ...
    def serialize_stats(self):
        return [
            ...
            if not (
                self.entries[key].num_requests == 0
                and self.entries[key].num_failures == 0
            )
        ]
...
class StatsEntry(object):
    ...
    def current_rps(self):
        ...
        slice_start_time = max(
            int(self.stats.last_request_timestamp) - 12, int(self.stats.start_time or 0)
        )

        reqs = [
            self.num_reqs_per_sec.get(t, 0)
            for t in range(slice_start_time, int(self.stats.last_request_timestamp) - 2)
        ]
        return avg(reqs)
    ...
    def total_rps(self):
        ...
        try:
            return self.num_requests / (
                self.stats.last_request_timestamp - self.stats.start_time
            )
        except ZeroDivisionError:
            return 0.0
    ...
    def get_response_time_percentile(self, percent):
        ...
        return calculate_response_time_percentile(
            self.response_times, self.num_requests, percent
        )
...
def print_percentile_stats(stats):
    ...
    headers = (
        ("Type", "Name")
        + tuple(get_readable_percentiles(PERCENTILES_TO_REPORT))
        + ("# reqs",)
    )
    console_logger.info(
        (
...
class StatsCSV:
    ...
    def _percentile_fields(self, stats_entry):
        return (
            [
                int(stats_entry.get_response_time_percentile(x) or 0)
                for x in self.percentiles_to_report
            ]
...
class StatsCSVFileWriter(StatsCSV):
    ...
    def __init__(
        self, environment, percentiles_to_report, base_filepath, full_history=False
    ):
