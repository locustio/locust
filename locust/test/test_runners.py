...
class TestLocustRunner(LocustTestCase):
    ...
    def test_weight_locusts(self):
        ...
        runner = LocustRunner(Environment(locust_classes=[L1, L2, L3], options=mocked_options()))
    ...
    def test_setup_method_exception(self):
        ...
        environment = Environment(locust_classes=[User], options=mocked_options())
    ...
    def test_reset_stats(self):
        ...
        environment = Environment(locust_classes=[User], reset_stats=True, options=mocked_options())
        runner = LocalLocustRunner(environment)
    ...
    def test_no_reset_stats(self):
        ...
        environment = Environment(locust_classes=[User], reset_stats=False, options=mocked_options())
        runner = LocalLocustRunner(environment)
        runner.start(locust_count=6, hatch_rate=12, wait=False)
        sleep(0.25)
        self.assertGreaterEqual(environment.stats.get("/test", "GET").num_requests, 3)
...
class TestMasterRunner(LocustTestCase):
    def setUp(self):
        ...
        class MyTestLocust(Locust):
            pass
        self.environment.locust_classes = [MyTestLocust]
    ...
    def test_spawn_zero_locusts(self):
        ...
        environment = Environment(locust_classes=[MyTestLocust], options=mocked_options())
        runner = LocalLocustRunner(environment)
...
class TestSlaveLocustRunner(LocustTestCase):
    ...
    def test_slave_stop_timeout(self):
        ...
        with mock.patch("locust.rpc.rpc.Client", mocked_rpc()) as client:
            environment = Environment(locust_classes=[MyTestLocust], options=mocked_options())
            slave = SlaveLocustRunner(environment)
    ...
    def test_slave_without_stop_timeout(self):
        ...
        with mock.patch("locust.rpc.rpc.Client", mocked_rpc()) as client:
            ...
            environment = Environment(locust_classes=[MyTestLocust], options=options)
            slave = SlaveLocustRunner(environment)
            self.assertEqual(1, len(client.outbox))
            self.assertEqual("client_ready", client.outbox[0].type)
            client.mocked_send(Message("hatch", {
    ...
    def test_change_user_count_during_hatching(self):
        ...
        with mock.patch("locust.rpc.rpc.Client", mocked_rpc()) as client:
            options = mocked_options()
            options.stop_timeout = None
            environment = Environment(locust_classes=[User], options=options)
            slave = SlaveLocustRunner(environment)
