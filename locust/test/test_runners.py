...
class TestLocustRunner(LocustTestCase):
    ...
    def test_weight_locusts(self):
        ...
        self.assert_locust_class_distribution(
            {L1: 10, L2: 9, L3: 10}, runner.weight_users(29)
        )
        self.assert_locust_class_distribution(
            {L1: 10, L2: 10, L3: 10}, runner.weight_users(30)
        )
        self.assert_locust_class_distribution(
            {L1: 11, L2: 10, L3: 10}, runner.weight_users(31)
        )
    ...
    def test_stop_users_with_spawn_rate(self):
        ...
        self.assertTrue(
            user_count > 5, "User count has decreased too quickly: %i" % user_count
        )
        self.assertTrue(
            user_count < 10, "User count has not decreased at all: %i" % user_count
        )
        ...
        self.assertTrue(
            user_count == 2,
            "User count has not decreased correctly to 2, it is : %i" % user_count,
        )
...
class TestMasterWorkerRunners(LocustTestCase):
    ...
    def test_distributed_shape(self):
        ...
        with mock.patch("locust.runners.WORKER_REPORT_INTERVAL", new=0.3):
            ...
            for worker in workers:
                self.assertEqual(
                    3, worker.user_count, "Shape test has not reached stage 1"
                )
            # Ensure new stage with more users has been reached
            sleep(2)
            for worker in workers:
                self.assertEqual(
                    7, worker.user_count, "Shape test has not reached stage 2"
                )
            # Ensure new stage with less users has been reached
            sleep(2)
            for worker in workers:
                self.assertEqual(
                    1, worker.user_count, "Shape test has not reached stage 3"
                )
    ...
    def test_distributed_shape_stop_and_restart(self):
        ...
        with mock.patch("locust.runners.WORKER_REPORT_INTERVAL", new=0.3):
            ...
            for worker in workers:
                self.assertEqual(
                    2, worker.user_count, "Shape test has not started correctly"
                )
            ...
            for worker in workers:
                self.assertEqual(
                    2, worker.user_count, "Shape test has not started again correctly"
                )
            master.stop()
    ...
    def test_last_worker_missing_stops_test(self):
        with mock.patch("locust.rpc.rpc.Server", mocked_rpc()) as server:
            ...
            server.mocked_send(
                Message(
                    "heartbeat",
                    {"state": STATE_RUNNING, "current_cpu_usage": 50},
                    "fake_client1",
                )
            )
    ...
    def test_master_current_response_times(self):
        ...
        with mock.patch("time.time") as mocked_time:
            ...
            with mock.patch("locust.rpc.rpc.Server", mocked_rpc()) as server:
                ...
                self.assertEqual(
                    400, master.stats.total.get_current_response_time_percentile(0.5)
                )
                self.assertEqual(
                    800, master.stats.total.get_current_response_time_percentile(0.95)
                )
                ...
                self.assertEqual(
                    30, master.stats.total.get_current_response_time_percentile(0.5)
                )
                self.assertEqual(
    ...
    def test_custom_shape_scale_up(self):
        ...
        with mock.patch("locust.rpc.rpc.Server", mocked_rpc()) as server:
            ...
            self.assertEqual(
                1,
                num_users,
                "Total number of users in first stage of shape test is not 1: %i"
                % num_users,
            )
            ...
            self.assertEqual(
                3,
                num_users,
                "Total number of users in second stage of shape test is not 3: %i"
                % num_users,
            )
            ...
            self.assertEqual(
                "stopped",
                master.state,
                "The test has not been stopped by the shape class",
            )
    ...
    def test_custom_shape_scale_down(self):
        ...
        with mock.patch("locust.rpc.rpc.Server", mocked_rpc()) as server:
            ...
            self.assertEqual(
                5,
                num_users,
                "Total number of users in first stage of shape test is not 5: %i"
                % num_users,
            )
            ...
            self.assertEqual(
                1,
                num_users,
                "Total number of users in second stage of shape test is not 1: %i"
                % num_users,
            )
...
class TestStopTimeout(LocustTestCase):
    def test_stop_timeout(self):
        ...
        # exit with timeout
        environment = Environment(
            user_classes=[MyTestUser], stop_timeout=short_time / 2
        )
        runner = environment.create_local_runner()
        runner.start(1, 1, wait=False)
        gevent.sleep(short_time)
        ...
        environment = Environment(
            user_classes=[MyTestUser], stop_timeout=short_time * 3
        )
        runner = environment.create_local_runner()
        runner.start(1, 1, wait=False)
        gevent.sleep(short_time)
        ...
        try:
            ...
            self.fail(
                "Got Timeout exception. Some locusts must have kept running after iteration finish"
            )
        finally:
            timeout.cancel()
