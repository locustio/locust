...
class TestRequestStats(unittest.TestCase):
    ...
    def test_percentile_rounded_down(self):
        ...
        self.assertEqual(
            actual_percentile,
            ["GET", "rounding", "down!"] + ["120"] * len(PERCENTILES_TO_REPORT) + ["1"],
        )
    ...
    def test_percentile_rounded_up(self):
        ...
        self.assertEqual(
            actual_percentile,
            ["GET", "rounding", "up!"] + ["130"] * len(PERCENTILES_TO_REPORT) + ["1"],
        )
    ...
    def test_custom_percentile_list(self):
        ...
        self.assertEqual(
            ...
            + ["150"] * len(custom_percentile_list)
            + ["1"],
        )
...
class TestCsvStats(LocustTestCase):
    ...
    def test_csv_stats_on_master_from_aggregated_stats(self):
        ...
        with mock.patch("locust.rpc.rpc.Server", mocked_rpc()) as server:
            ...
            master = environment.create_master_runner(
                master_bind_host="*", master_bind_port=0
            )
            greenlet = gevent.spawn(stats_writer)
            gevent.sleep(_TEST_CSV_STATS_INTERVAL_WAIT_SEC)
...
class TestStatsEntry(unittest.TestCase):
    ...
    def test_fail_ratio_with_no_failures(self):
        ...
        self.assertAlmostEqual(
            output_fields["failure_percentage"], EXPECTED_FAIL_RATIO * 100
        )
...
class TestInspectUser(unittest.TestCase):
    def test_get_task_ratio_dict_relative(self):
        ...
        self.assertEqual(
            0.5, ratio["MyTaskSet"]["tasks"]["MySubTaskSet"]["tasks"]["task1"]["ratio"]
        )
        self.assertEqual(
            0.5, ratio["MyTaskSet"]["tasks"]["MySubTaskSet"]["tasks"]["task2"]["ratio"]
        )
    ...
    def test_get_task_ratio_dict_total(self):
        ...
        self.assertEqual(
            0.125,
            ratio["MyTaskSet"]["tasks"]["MySubTaskSet"]["tasks"]["task1"]["ratio"],
        )
        self.assertEqual(
            0.125,
            ratio["MyTaskSet"]["tasks"]["MySubTaskSet"]["tasks"]["task2"]["ratio"],
        )
