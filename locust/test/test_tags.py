...
class TestTags(LocustTestCase):
    ...
    def test_tagging_taskset(self):
        ...
        self.assertEqual(
            set(["taskset", "task"]), MyTaskSet.tagged_again.locust_tag_set
        )

        # when tagging nested taskset, its tasks receives both
        self.assertIn("locust_tag_set", dir(MyTaskSet.NestedTaskSet.nested_task))
        self.assertEqual(
            set(["taskset", "taskset2"]),
            MyTaskSet.NestedTaskSet.nested_task.locust_tag_set,
        )
    ...
    def test_including_tasksets(self):
        ...
        self.assertListEqual(
            MyTaskSet.MixedNestedTaskSet.tasks, [MyTaskSet.MixedNestedTaskSet.included]
        )
    ...
    def test_exclude_tags_under_user(self):
        ...
        self.assertListEqual(
            MyUser.tasks, [MyUser.not_excluded, MyUser.dont_exclude_this_either]
        )
