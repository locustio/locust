...
class TestWebUI(LocustTestCase, _HeaderCheckMixin):
    ...
    def test_index(self):
        self.assertEqual(
            200, requests.get("http://127.0.0.1:%i/" % self.web_port).status_code
        )
    ...
    def test_stats_no_data(self):
        self.assertEqual(
            200,
            requests.get(
                "http://127.0.0.1:%i/stats/requests" % self.web_port
            ).status_code,
        )
    ...
    def test_request_stats_csv(self):
        ...
        response = requests.get(
            "http://127.0.0.1:%i/stats/requests/csv" % self.web_port
        )
        self.assertEqual(200, response.status_code)
    ...
    def test_request_stats_full_history_csv_not_present(self):
        self.stats.log_request("GET", "/test2", 120, 5612)
        response = requests.get(
            "http://127.0.0.1:%i/stats/requests_full_history/csv" % self.web_port
        )
...
class TestWebUIAuth(LocustTestCase):
    ...
    def test_index_with_basic_auth_enabled_correct_credentials(self):
        self.assertEqual(
            200,
            requests.get(
                "http://127.0.0.1:%i/?ele=phino" % self.web_port, auth=("john", "doe")
            ).status_code,
        )
    ...
    def test_index_with_basic_auth_enabled_incorrect_credentials(self):
        self.assertEqual(
            401,
            requests.get(
                "http://127.0.0.1:%i/?ele=phino" % self.web_port,
                auth=("john", "invalid"),
            ).status_code,
        )
    ...
    def test_index_with_basic_auth_enabled_blank_credentials(self):
        self.assertEqual(
            401,
            requests.get("http://127.0.0.1:%i/?ele=phino" % self.web_port).status_code,
        )
...
class TestWebUIWithTLS(LocustTestCase):
    def setUp(self):
        ...
        self.stats = self.runner.stats
        self.web_ui = self.environment.create_web_ui(
            "127.0.0.1", 0, tls_cert=options.tls_cert, tls_key=options.tls_key
        )
        gevent.sleep(0.01)
        self.web_port = self.web_ui.server.server_port
    ...
    def test_index_with_https(self):
        ...
        self.assertEqual(
            200,
            requests.get(
                "https://127.0.0.1:%i/" % self.web_port, verify=False
            ).status_code,
        )
...
class TestWebUIFullHistory(LocustTestCase, _HeaderCheckMixin):
    ...
    def setUp(self):
        ...
        parser = get_parser(default_config_files=[])
        self.environment.parsed_options = parser.parse_args(
            ["--csv", self.STATS_BASE_NAME, "--csv-full-history"]
        )
        self.stats = self.environment.stats
        ...
        self.web_ui = self.environment.create_web_ui(
            "127.0.0.1", 0, stats_csv_writer=self.stats_csv_writer
        )
        self.web_ui.app.view_functions["request_stats"].clear_cache()
        gevent.sleep(0.01)
        self.web_port = self.web_ui.server.server_port
