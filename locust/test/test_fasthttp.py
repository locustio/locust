...
class TestFastHttpSession(WebserverTestCase):
    def get_client(self):
        return FastHttpSession(
            self.environment, base_url="http://127.0.0.1:%i" % self.port
        )
    ...
    def test_connection_error(self):
        ...
        self.assertTrue(
            isinstance(
                next(iter(self.runner.stats.errors.values())).error,
    ...
    def test_404(self):
        ...
        self.assertEqual(
            1, self.runner.stats.get("/does_not_exist", "GET").num_failures
        )
    ...
    def test_catch_response_fail_successful_request(self):
        ...
        with s.get("/ultra_fast", catch_response=True) as r:
            r.failure("nope")
        self.assertEqual(
            1, self.environment.stats.get("/ultra_fast", "GET").num_requests
        )
        self.assertEqual(
            1, self.environment.stats.get("/ultra_fast", "GET").num_failures
        )
    ...
    def test_catch_response_default_success(self):
        ...
        self.assertEqual(
            0, self.environment.stats.get("/ultra_fast", "GET").num_failures
        )
...
class TestRequestStatsWithWebserver(WebserverTestCase):
    def test_request_stats_content_length(self):
        ...
        self.assertEqual(
            self.runner.stats.get("/ultra_fast", "GET").avg_content_length,
    ...
    def test_request_stats_query_variables(self):
        ...
        self.assertEqual(
            1, self.runner.stats.get("/ultra_fast?query=1", "GET").num_requests
        )
...
class TestFastHttpUserClass(WebserverTestCase):
    ...
    def test_network_timeout_setting(self):
        ...
        self.assertEqual(
            1,
            self.runner.stats.get(
                "/redirect?url=/redirect&delay=5.0", "GET"
            ).num_failures,
        )
    ...
    def test_allow_redirects_override(self):
        ...
        self.assertEqual(
            "http://127.0.0.1:%i/ultra_fast" % self.port, resp.headers["location"]
        )
...
class TestFastHttpSsl(LocustTestCase):
    ...
    def test_ssl_request_insecure(self):
        s = FastHttpSession(
            self.environment, "https://127.0.0.1:%i" % self.web_port, insecure=True
        )
        r = s.get("/")
        self.assertEqual(200, r.status_code)
